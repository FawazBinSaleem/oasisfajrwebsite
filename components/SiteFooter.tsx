    ...
    <footer className="bg-ink text-ink-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-14 lg:grid-cols-4 lg:px-8 lg:py-16">
        <div className="lg:col-span-1">
          <img src={company.logo} alt={`${company.name} logo`} className="h-12 w-auto" />
        ...
        <nav aria-label="Footer navigation">
          <h2 className="eyebrow text-ink-foreground/50">Navigate</h2>
          <ul className="mt-5 space-y-2.5">
            {mainNav.map((item) => (
              <li key={item.to}>
                <Link to={item.to} className="text-sm text-ink-foreground/75 hover:text-primary">
                  {item.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-5 space-y-2.5 text-sm text-ink-foreground/75">
            ...
            <li>
              <a href={company.whatsapp} target="_blank" rel="noreferrer" className="hover:text-primary">
